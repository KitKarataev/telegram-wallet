...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Settings (RLS automatically filters by user_id)
        settings_res = (
        ...
        # OPTIMIZATION: Use database aggregation instead of fetching all records
        try:
            balance_res = supabase.rpc('calculate_balance', {'p_user_id': user_id}).execute()
            if balance_res.data and len(balance_res.data) > 0:
                total_balance = float(balance_res.data[0].get('balance', 0))
            else:
                total_balance = 0.0
        except Exception as e:
            # Fallback to manual calculation if function doesn't exist yet
            print(f"Warning: calculate_balance function not found, using fallback: {e}")
            all_data = (
        ...
        # Recent records for history and period stats
        all_data = (
            supabase.table("expenses")
            .select("*")
            .order("created_at", desc=True)
        ...
        # Subscriptions (RLS filters automatically)
        subs_res = (
        ...
        # Sort chart categories by spend desc
